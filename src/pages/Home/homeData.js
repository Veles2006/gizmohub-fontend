import {
    faCartShopping,
    faMedal,
    faShield,
    faTag,
} from '@fortawesome/free-solid-svg-icons';

const data = {
    values: [
        {
            header: 'GoPro hero9 Black',
            description: 'Limited stocks available. Grab it now!',
            button: 'Shop Collection',
            image: '/images/banner-image-1.png',
        },
        {
            header: 'Iphone 15 Pro Max',
            description: 'Discount available. Grab it now!',
            button: 'Shop Product',
            image: '/images/banner-image-2.png',
        },
        {
            header: 'Macbook Collection',
            description: 'Limited stocks available. Grab it now!',
            button: 'Shop Collection',
            image: '/images/banner-image-3.png',
        },
    ],

    companyServices: [
        {
            header: 'Free delivery',
            content: 'Consectetur adipi elit lorem ipsum dolor sit amet.',
            icon: faCartShopping,
        },
        {
            header: 'Quality guarantee',
            content: 'Dolor sit amet orem ipsu mcons ectetur adipi elit.',
            icon: faMedal,
        },
        {
            header: 'Daily offers',
            content: 'Amet consectetur adipi elit loreme ipsum dolor sit.',
            icon: faTag,
        },
        {
            header: '100% secure payment',
            content: 'Rem Lopsum dolor sit amet, consectetur adipi elit.',
            icon: faShield,
        },
    ],

    categories: [
        {
            title: 'Phones',
            img: 'https://demo.templatesjungle.com/shoplite/images/cat-item1.png',
        },
        {
            title: 'PlayStations',
            img: 'https://demo.templatesjungle.com/shoplite/images/cat-item2.png',
        },
        {
            title: 'Digital watches',
            img: 'https://demo.templatesjungle.com/shoplite/images/cat-item3.png',
        },
        {
            title: 'Joysticks',
            img: 'https://demo.templatesjungle.com/shoplite/images/cat-item4.png',
        },
        {
            title: 'EarPods',
            img: 'https://demo.templatesjungle.com/shoplite/images/cat-item5.png',
        },
        {
            title: 'Laptops',
            img: 'https://demo.templatesjungle.com/shoplite/images/cat-item6.png',
        },
    ],

    bestSellingItems: [
        {
            title: 'IPad (9th Gen)',
            price: '$870',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item1.png',
        },
        {
            title: 'Drone With Camera',
            price: '$600',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item2.png',
        },
        {
            title: 'Apple Watch (2nd Gen)',
            price: '$400',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item3.png',
        },
        {
            title: 'Ultra HD TV',
            price: '$2000',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item4.png',
        },
        {
            title: 'Bluetooth Speaker',
            price: '$75',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item5.png',
        },
        {
            title: 'White Headset',
            price: '$99',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item6.png',
        },
        {
            title: 'Black Bluetooth Speaker',
            price: '$80',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item7.png',
        },
        {
            title: 'Large Speaker',
            price: '$450',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item8.png',
        },
        {
            title: 'White EarPods',
            price: '$600',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item9.png',
        },
        {
            title: 'Laptop',
            price: '$1200',
            img: 'https://demo.templatesjungle.com/shoplite/images/product-item10.png',
        },
    ],

    itemsListing: [
        {
            header: 'Featured',
            products: [
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
            ],
        },
        {
            header: 'Featured',
            products: [
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
            ],
        },
        {
            header: 'Featured',
            products: [
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
            ],
        },
        {
            header: 'Featured',
            products: [
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
                {
                    title: 'Wireless headset',
                    price: '$500',
                    img: 'https://demo.templatesjungle.com/shoplite/images/item-image1.jpg',
                },
            ],
        },
    ],
    customersReviews: [
        {
            id: 0,
            comment:
                "“I stumbled upon this tech store while searching for a new laptop, and I couldn't be happier with my experience! The staff was incredibly knowledgeable and guided me through the process of choosing the perfect device for my needs. Highly recommended!”",
            stars: 5,
            user: 'Emma Chamberlin',
        },
        {
            id: 1,
            comment:
                "“This tech store is my go-to for all things tech! Whether it's a new smartphone, accessories, or even troubleshooting advice, they've always got me covered. The staff is friendly, and their expertise is unmatched. Trust me; you won't regret shopping here!”",
            stars: 5,
            user: 'Thomas John',
        },
        {
            id: 2,
            comment:
                "“I recently purchased a smartwatch from this tech store, and I'm absolutely thrilled find the perfect fit with my purchase! Not only did they have an extensive find the perfect fit selection to choose from, but their team helped me find the perfect fit for my lifestyle.”",
            stars: 5,
            user: 'Kevin Bryan',
        },
        {
            id: 3,
            comment:
                "“I stumbled upon this tech store while searching for a new laptop, and I couldn't be happier with my experience! The staff was incredibly knowledgeable and guided me through the process of choosing the perfect device for my needs. Highly recommended!”",
            stars: 5,
            user: 'Stevin',
        },
        {
            id: 4,
            comment:
                "“I stumbled upon this tech store while searching for a new laptop, and I couldn't be happier with my experience! The staff was incredibly knowledgeable and guided me through the process of choosing the perfect device for my needs. Highly recommended!”",
            stars: 5,
            user: 'Roman',
        },
    ],
    latestPosts: [
        {
            flag: 'Gadgets',
            title: '5 Must-Have Gadgets for the Modern Home',
            paragraph:
                'Dive into the world of cutting-edge technology with our latest blog post, where we highlight five essential gadg',
            image: 'https://demo.templatesjungle.com/shoplite/images/post-item1.jpg',
        },
        {
            flag: 'Phones',
            title: 'Eco-Friendly Innovations Making a Difference',
            paragraph:
                'Explore the intersection of technology and sustainability in our latest blog post. Learn about the innovative',
            image: 'https://demo.templatesjungle.com/shoplite/images/post-item2.jpg',
        },
        {
            flag: 'Tech',
            title: 'The Future of Wearable Tech: Trends to Watch',
            paragraph:
                'Stay ahead of the curve with our insightful look into the rapidly evolving landscape of wearable technology.',
            image: 'https://demo.templatesjungle.com/shoplite/images/post-item3.jpg',
        },
        {
            flag: 'Digital Watch',
            title: 'Top Apps and Tools for Remote Work',
            paragraph:
                "In today's remote work environment, productivity is key. Discover the top apps and tools that can help you stay",
            image: 'https://demo.templatesjungle.com/shoplite/images/post-item4.jpg',
        },
    ],
    brands: [
        'https://demo.templatesjungle.com/shoplite/images/brand1.png',
        'https://demo.templatesjungle.com/shoplite/images/brand2.png',
        'https://demo.templatesjungle.com/shoplite/images/brand3.png',
        'https://demo.templatesjungle.com/shoplite/images/brand4.png',
        'https://demo.templatesjungle.com/shoplite/images/brand5.png',
    ],
    instagram: [
        'https://demo.templatesjungle.com/shoplite/images/insta-item1.jpg',
        'https://demo.templatesjungle.com/shoplite/images/insta-item2.jpg',
        'https://demo.templatesjungle.com/shoplite/images/insta-item3.jpg',
        'https://demo.templatesjungle.com/shoplite/images/insta-item4.jpg',
        'https://demo.templatesjungle.com/shoplite/images/insta-item5.jpg',
        'https://demo.templatesjungle.com/shoplite/images/insta-item6.jpg',
    ],
};

export default data;
